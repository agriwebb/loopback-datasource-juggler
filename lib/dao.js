          ...
          Model.notifyObserversOf('loaded', context, function(err) {
            ...
            saveDone.call(obj, function() {
              createDone.call(obj, function() {
                ...
                // We can only defer after save hooks if the saved model has an id
                if (options.deferAfterSave && obj[_idName]) {
                  if (!options.afterSaveContextMap) {
                    options.afterSaveContextMap = {};
                  }
                  options.afterSaveContextMap[obj[_idName]] = context;
                  cb(null, obj);
                } else {
        ...
        function done(err, data, info) {
          ...
          Model.notifyObserversOf('loaded', context, function(err) {
            ...
            if (err) {
              ...
              if (options.deferAfterSave) {
                if (!options.afterSaveContextMap) {
                  options.afterSaveContextMap = {};
                }
                options.afterSaveContextMap[id] = context;
                cb(null, obj);
              } else {
                Model.notifyObserversOf('after save', context, function(err) {
                  cb(err, obj);
